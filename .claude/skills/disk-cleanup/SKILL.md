...
| HTTP disk cache | `cache2/*` in Firefox profile | `rm -rf` (Firefox auto-regenerates on restart) |
| `C:\Windows\LiveKernelReports\*.dmp` | `WATCHDOG-*.dmp`, kernel crash dumps | Elevated delete (see below) |
| `%TEMP%\wsl-crashes\*` | WSL2 WebKit/GPU crash dumps (5+ GB each) | `rm -rf` — post-mortem only, safe to delete |
| `%TEMP%\DiagOutputDir\` | Windows diagnostic trace output (multi-GB) | `rm -rf` — auto-regenerated by diagnostic tools |
| OS/development installer files | `cuda-repo-*-local_*.deb`, `*.msi`, `*.exe` in dev dirs | `rm` — installed OS packages no longer needed as files |
| ML model caches | `%USERPROFILE%\.cache\huggingface\hub\` | `rm -rf` — models re-download on next use |
| Abandoned tool data | `%USERPROFILE%\.lingma\` (灵码), old IDE plugins | `rm -rf` — only if user confirms tool is no longer used |
...
### Temp Folder Deep-Dive

`%TEMP%` (aka `%LOCALAPPDATA%\Temp`) is the single highest-ROI target during cleanup — it can balloon from 1.5 GB in a week-old report to 25 GB on the live filesystem. **Always check Temp's current size with `du -sh`, not just the report snapshot.**

Priority order within Temp:

| Priority | Pattern | Why |
|----------|---------|-----|
| 1 | `wsl-crashes\` | WSL2 crash dumps (WebKit, GPU) — single files can reach 5.5 GB; purely diagnostic |
| 2 | `DiagOutputDir\` | Windows diagnostic trace — multi-GB, auto-regenerated |
| 3 | `Diagnostics\` | Smaller diagnostic output, same logic |
| 4 | `*.tmp` | Individual temp files; many will be **locked** by running processes → skip those, delete the rest |
| 5 | `trae-user-x64\`, `claude\`, `DockerDesktop\` | In-use tool temp files — **skip**, tool may malfunction |

**`.tmp` locking:** Running processes hold exclusive locks on their active `.tmp` files. `rm -f *.tmp` will report "Device or resource busy" for these — this is normal and expected. The locked files will be cleaned up when the owning process exits. Don't try to force-delete them.
...
**Scenario D — Duplicate docker_data.vhdx from nested path config:**

When a SpaceSniffer report shows two `docker_data.vhdx` at different nesting levels:

```
D:\DockerData\DockerDesktopWSL\
├── disk\docker_data.vhdx              ← stale (3.7 GB)
├── main\                              ← empty, no ext4.vhdx
└── DockerDesktopWSL\                  ← active target per config
    ├── disk\docker_data.vhdx          ← in-use (25.6 GB)
    └── main\ext4.vhdx                 ← system engine
```

**Detection:**

1. Read Docker settings: `%APPDATA%\Docker\settings.json` → `customWslDistroDir`
2. Compare `LastWriteTime` of both `docker_data.vhdx` files — the one with the **newer timestamp** is the active one
3. If the outer `main\` lacks `ext4.vhdx`, the outer path was abandoned
...
# 2. Rename (not delete) to test
Rename-Item "D:\DockerData\DockerDesktopWSL\disk\docker_data.vhdx" docker_data.vhdx.bak

# 3. Restart Docker, verify containers work
# 4. Delete .bak only after confirming Docker is healthy
Remove-Item "D:\DockerData\DockerDesktopWSL\disk\docker_data.vhdx.bak"
```

**Root cause:** The `customWslDistroDir` was changed from `D:\DockerData\DockerDesktopWSL` to `D:\DockerData\DockerDesktopWSL\DockerDesktopWSL` (one level deeper), and Docker created a fresh VHDX at the new path without cleaning up the old one.
...
| Abandoned tool data (`.lingma`, `.trae`, old IDE versions) | User may return to the tool | Ask user: "Do you still use X?" If no, `rm -rf` |
| ML model caches (HuggingFace hub) | Re-download can be slow | Safe to delete but warn: models will re-download on next use |
| Git LFS cache in ML model git repos (`.git/lfs/objects/`) | Contains versioned copy of model weights; duplicates working-tree model file | Ask user: "Is this model still needed?" If no, delete entire repo directory; the LFS cache cannot be deleted independently without breaking git integrity |
...
1. **Check Temp folder live** — `du -sh %TEMP%` — SpaceSniffer reports are stale; Temp can grow 10× between scans. Delete crash dumps, diagnostic output, and unlocked `.tmp` files first (see [Temp Folder Deep-Dive](#temp-folder-deep-dive))
2. Start with the **largest** report item first (biggest win)
3. **Check Docker config for stale data** — read `%APPDATA%\Docker\settings.json` → `customWslDistroDir`; look for duplicate `docker_data.vhdx` files at different nesting levels (see [Scenario D](#scenario-d--duplicate-docker_data-vhdx-from-nested-path-config))
4. **Check dev directories for OS installers** — look for large `.deb`, `.exe`, `.msi` files in development trees; ask user before deleting
5. Delete safe items immediately
6. Migrate eligible items one at a time (copy → verify → junction → confirm)
7. Ask user for cautious items (abandoned tools, ML caches, user data, Git LFS caches)
8. Re-scan after major changes to confirm space freed
...
| SpaceSniffer report shows Temp as 1.5 GB but `du` shows 25 GB | Temp is highly volatile — always check live filesystem, not the report snapshot |
| CUDA `.deb` installer (3+ GB) in dev directories | Safe to delete after installation; verify file is an installer (`.deb`, `.exe`, `.msi`), not source code |
| `rm -f *.tmp` fails with "Device or resource busy" | Normal — running processes lock their temp files. Skip locked files; they auto-cleanup on process exit |
| `rm -rf` on `wsl-crashes\` succeeds but `du` still shows usage | `wsl-crashes\` may be symlinked or have hidden streams; verify with `ls -la` and retry |
| HuggingFace cache deletion causes slow next model load | Warn user that models will re-download; consider keeping frequently-used models |
| Duplicate `docker_data.vhdx` at different nesting levels | Check Docker's `customWslDistroDir` in `settings.json`; compare `LastWriteTime` to identify stale copy |
| Git LFS objects in ML model repos (`.git/lfs/objects/`) | LFS cache ≈ model file size; ask user before deleting — model may still be in use |
